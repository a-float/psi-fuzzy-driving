# car initialization, board drawing and the main loop
...
fuzzy_car1 = Car(randomize_pos(CENTER, 40), random() * 2 * math.pi, (255, 0, 0))
fuzzy_car2 = Car(randomize_pos(CENTER, 30), random() * 2 * math.pi, (0, 255, 255))
...
def draw_map():
    for row in range(MAP_SIZE[1]):
        for col in range(MAP_SIZE[0]):
            pygame.draw.rect(
                win,
                (180, 180, 180) if MAP[row][col] == '#' else (100, 100, 100),
...
while True:
    ...
    pygame.draw.rect(win, (40, 40, 40), (0, 0, SCREEN_WIDTH, SCREEN_HEIGHT))
    draw_map()
